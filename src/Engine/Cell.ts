...
export class Cell {
  ...
  //TODO: add a field to see if this cell can be edited or not
   ...
   * 
   * */
  constructor(cell?: Cell, formula?: string[], value?: number, error?: string, displayString?: string, dependsOn?: string[], label?: string, editable?: boolean) {
    if (cell || cell !== undefined) {
      ...
      this._editable = cell._editable;

    } else {
      // default constructor logic
      this._formula = formula ? [...formula] : [];
      this._value = value ? value : 0;
      //this._error = error ? error : ErrorMessages.emptyFormula;
      this._error = error ? error : "";
      this._displayString = displayString ? displayString : "";
      this._dependsOn = dependsOn ? [...dependsOn] : [];
