...
export class FormulaBuilder {
  ...
  addToken(token: TokenType): void {
    ...
    // //if the last token is a number and the input token is "+/-", then modify the last token to be the opposite sign
    // if (!isNaN(Number(lastToken)) && token === "+/-") { 
    //   lastToken = (Number(lastToken) * -1).toString();  
    //   lastTokenUpdated = true;
    // }

    // //if the last token is a number but not zero, and the input token is "1/x", then modify the last token to be the 1/lastToken
    // if (!isNaN(Number(lastToken)) && token === "1/x" && Number(lastToken) !== 0) {
    //   lastToken = (1 / Number(lastToken)).toString(); 
    //   lastTokenUpdated = true;
    // }
