...
export class SpreadSheetController {
   ...
   * constructor
   * */
  // constructor(columns?: number, rows?: number, sheet?: any) {
  //   if(columns !== undefined && rows !== undefined && sheet === undefined){
  //     console.log("Creating new sheet 1111");
  //     this._memory = new SheetMemory(columns, rows);
  //   }else {
  //     console.log("Creating new sheet 2222");
  //     this._memory = new SheetMemory(sheet._numColumns, sheet._numRows, sheet._cells, sheet._currentColumn, sheet._currentRow, sheet._ssId);
  //   }
  ...
  public getEditStatusString(): string {
    if (this._cellIsBeingEdited) {
      ...
      return "editing: " + this.getCurrentCell().getLabel();
    }
    //return "current cell: " + this.getWorkingCellLabel();
    return "current cell: " + this.getCurrentCell().getLabel();
  }

  /**
   * 
   * @returns Get the current cell
   */
  public getCurrentCell(): Cell {
    return this._memory.getCurrentCell();
  }

  /**
   * Set the sheet memory object
   */
  public setSheetMemory(sheetMemory: any): void { 
    this._memory = sheetMemory;
  }

  /**
   * Get the sheetMemory object
   * 
   */
  public getSheetMemory(): SheetMemory {
    return this._memory;
  }

  /**
   * Set the formula builder for the controller
   * 
   */
  public setFormulaBuilder(formulaBuilder: FormulaBuilder): void {
    this._formulaBuilder = formulaBuilder;
  }

  /**
   * Get the current row for the controller
   */
  public getCurrentRow(): number {
    return this._currentWorkingRow;
  }

  /**
   * Get the current column for the controller
   *  
   * @returns number
   *  
   * */
  public getCurrentColumn(): number {
    return this._currentWorkingColumn;
  }
   ...
   * 
   */
  public getCalculationManager(): CalculationManager {
